...
import { Key, Sparkles, ChevronRight, ArrowLeft } from 'lucide-react';
...
export default function Onboarding({
  ...
  const handleValidate = async (e: React.FormEvent) => {
    ...
    try {
      // Execute backend API key validation check
      const res = await api.validateKey(keyInput);
      if (res.valid) {
        setStatus('success');
        setTimeout(() => {
        ...
        throw new Error('Key handshake check rejected.');
      }
    } catch (err: any) {
      setErrorMessage(err.message || 'Key validation handshake failed. Verify your key in Google AI Studio.');
      setStatus('error');
    }
  };
    ...
    <div className="flex min-h-[75vh] items-center justify-center px-6 py-12" id="onboarding-page">
      <div className="w-full max-w-md rounded-xl border border-[#1b1e2c] bg-[#0e1017] p-8 shadow-[0_8px_30px_rgb(0,0,0,0.5)]">
        ...
        <button 
          ...
          className="inline-flex items-center gap-1.5 text-xs text-[#5b647f] hover:text-[#8b949e] transition mb-6 cursor-pointer"
        ...
        {status === 'error' && errorMessage && (
          <div className="mb-4 flex items-start gap-2.5 rounded border border-red-500/20 bg-red-500/5 p-3 text-xs text-red-400 font-mono">
        ...
        <form onSubmit={handleValidate} className="space-y-4">
          <div>
            ...
            <p className="mt-2 text-[10px] text-[#5b647f] leading-relaxed">
              Your API key is saved locally in your browser cache. It is passed via headers to route your custom requests safely.
            </p>
          </div>
          ...
          >
            {status === 'validating' ? (
              <span className="flex items-center gap-2">
                ...
                <span className="font-mono text-[11px]">gemini-2.5: handshake_check()...</span>
              </span>
            ) : status === 'success' ? (
