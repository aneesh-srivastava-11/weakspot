...
import { ShieldAlert, Code2, Terminal, ArrowRight, Eye, EyeOff } from 'lucide-react';
...
export default function LoginSignup({ onLoginSuccess, initialEmail = '' }: LoginSignupProps) {
      ...
      <div className="w-full max-w-sm rounded-xl border border-[#1b1e2c] bg-[#0e1017] p-8 shadow-[0_8px_30px_rgb(0,0,0,0.5)]">
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              ...
              {isLogin && (
                <button
                  type="button"
                  className="font-sans text-[10px] text-[#5b647f] hover:text-[#8b949e] transition"
                  onClick={() => setError('Password resets are simulated. Feel free to just click Sign In!')}
                >
                  Forgot?
                </button>
              )}
            ...
            <div className="relative">
              ...
              <button
                type="button"
                className="absolute right-3 top-2.5 text-[#5b647f] hover:text-[#8b949e]"
        ...
        <div className="mt-6 text-center">
          <span className="font-mono text-[9px] text-[#3f445b]">
            SECURE SHA-256 SESSION ENCRYPTION ACTIVE
          </span>
        </div>
      </div>
    </div>
